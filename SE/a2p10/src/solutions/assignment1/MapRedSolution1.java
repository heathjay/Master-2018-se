...
public class MapRedSolution1
{
    ...
    //Map
	  public static class MapRecords extends Mapper<LongWritable, Text, Text, IntWritable>
     {
         ...
         private Text URLUnit = new Text();
        
         @Override
         protected void map(LongWritable key, Text value, Context context)  throws IOException, InterruptedException
         {
         
                 String InputStringValue = value.toString();
                 String[] URLData = InputStringValue.split(" ");
                 String data = URLData[6];
                 URLUnit.set(data);
                 context.write(URLUnit, one);
